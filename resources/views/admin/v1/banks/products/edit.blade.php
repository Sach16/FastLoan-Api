...
@section('main')
    <h1>Edit product document</h1>
    @include('admin.v1.layouts.partials._messages')
    <hr>
    <form
            enctype="multipart/form-data"
            action="{{ route('admin.v1.banks.products.update', [
                'banks' => $bank, 'attachments' => $attachment->id
            ]) }}"
        ...
        <div class="form-group col-md-9 {{ hasErrors('product', $errors) }}">
            ...
            <input name="product" type="hidden" value="{{ $product }}" id="selected-loan-types">
            <select name="product" id="inputProduct" class="form-control" data-loan-types disabled></select>
        </div>
