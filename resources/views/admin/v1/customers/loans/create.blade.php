...
@section('main')
    <h1>Add new Loan <small>({{ $customer->present()->name }})</small></h1>
    @include('admin.v1.layouts.partials._messages')
    <hr>
    <form action="{{ route('admin.v1.customers.loans.store', $customer->id) }}"
    ...
    {{--Status --}}
    <div class="form-group col-md-8 {{ hasErrors('loan_status_id', $errors) }}">
    ...
    <div class="form-group col-md-8 {{ hasErrors('appid', $errors) }}">
        ...
        <input type="text" class="form-control" id="inputAppid" name="appid" value="{{ old('appid') }}" >
    </div>
    ...
    <div class="form-group col-md-8 {{ hasErrors('property_verified', $errors) }}" id="divPropertyVerified" {{ old('type_id') !='1'?'hidden':'' }}>
        ...
        <select name="property_verified" id="inputPropertyVerified" class="form-control" data-chosen @if(!in_array(old('type_id'),[1,0])){{ 'disabled' }} @endif>
