...
@section('main')
    ...
    <hr>
        <form action="{{ route('admin.v1.teams.remove.from.team', [$team->id,$user->id]) }}" class="form-horizontal" method="post">
        {{ csrf_field() }}

        {{--Reports To--}}
        <div class="form-group col-md-8 {{ hasErrors('assigned_to', $errors) }}">
            <label class="control-label" for="inputIncentiveEarned">Assign customers to another DSA</label>
            <select  class="form-control" id="inputIncentiveEarned" name="assigned_to" data-chosen>
            <option value='' selected disabled >Select member</option>
            @foreach($team->members as $member)
                <option value="{{ $member->id }}">{{ $member->present()->name }}</option>
            @endforeach
            </select>
        </div>
